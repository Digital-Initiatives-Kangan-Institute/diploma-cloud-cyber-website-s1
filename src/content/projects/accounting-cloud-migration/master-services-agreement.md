...
description: 'Master Services Agreement between YAT College and MTS Consulting governing the Accounting System (Ledgerline) Cloud Migration project.'
appearsIn:
  - s1-cl1-at1
  - s1-cl1-at2
  - s1-cl1-at3
...
| | |
|---|---|
| **Agreement reference** | YAT-MTS-2026-004 |
| **Effective date** | 4 March 2026 |
| **Project** | Accounting System Cloud Migration |
...
A. YAT operates a self-hosted on-premises Accounting & Office Administration system (the **Accounting System**, the **Ledgerline** platform) at the YAT Cremorne campus, running on Windows Server 2016 with a Microsoft SQL Server database on the Application Services server.

B. The Application Services server is approaching the end of its warranty and capacity headroom. Consistent with its current ICT Strategic Plan and its direction of reducing dependency on in-house server infrastructure, YAT has determined to evaluate the future operating model for the Accounting System — renew on-premises versus migrate to a cloud-hosted environment — and, on an approved business case, to migrate the system to the cloud.

C. YAT has determined that the cloud infrastructure work component — the analysis and, on approval, the cloud foundation build — is to be delivered by an external consultancy under a Master Services Agreement, while the Ledgerline application installation, financial-data migration, cutover, and organisational change-management components are to be retained and delivered by YAT in-house ICT and Finance / Administration teams.

D. The Accounting System is an internal, business-hours system reached by finance and administrative staff (payroll is outsourced to an external bureau). YAT requires the recommended option to meet a cost-proportionate availability target appropriate to a business-hours finance system, to be confirmed during requirements analysis.
...
|---|---|---|
| Phase 1 | Analysis and Business Case | Strategic analysis of YAT's current ICT environment against its strategic plan; evaluation of the operating-model options (renew on-premises versus migrate to AWS); a five-year cost-benefit analysis with explicit treatment of the system's commercial licensing; recommendation and prioritised action plan; preparation of a board-ready Business Case and supporting presentation. |
| Phase 2 | Cloud Foundation Build | Implementation of the cloud architecture recommended in Phase 1 and approved by YAT: identity and access management, virtual network, compute, managed database for Microsoft SQL Server, internal application-delivery and storage, and a monitoring baseline; infrastructure smoke testing; documentation and handover. |

### 4.2 Detailed scope

Detailed scope for each Phase is set out in the supporting project documents:

- **Phase 1 scope** is anchored by the Engagement Role Brief, the Migration Requirements, the ICT Manager Consultation Notes, the Accounting System Operational Costing (cost baseline), and the ICT Strategic Plan (all on the YAT intranet for this project).
- **Phase 2 scope** is anchored by the action plan and architecture approved at the conclusion of Phase 1, the Accounting System Application Specification, the Migration Requirements, and the Accounting System Cloud Architecture — Baseline Design.
...
- Installation and configuration of the Ledgerline application onto the cloud infrastructure provided by MTS;
- Extract, transform, and load of the existing Microsoft SQL Server financial database from the on-premises system to the cloud-hosted managed database, including post-migration data verification and reconciliation;
- Production cutover from the on-premises system to the cloud-hosted system, including DNS changes, dual-running window operations, user redirection, and post-cutover incident response — scheduled to avoid the Restricted Period;
- All organisational change-management activities, including Change Advisory Board process, communications to staff, finance-team training collateral preparation and delivery, and post-cutover end-user support;
- Decommissioning of the on-premises Application Services server hardware and ancillary equipment, including secure data destruction; and
- All routine end-user support, helpdesk, and YAT-side operational ICT activities.

This out-of-scope allocation is reflected in the YAT-internal cost categories of the Accounting System Operational Costing for this project. Any change to this allocation requires a Change Request under clause 9.
...
| D1.2 | Five-year Cost-Benefit Analysis (renew on-premises versus migrate to AWS), with explicit treatment of commercial licensing | Completed CBA workbook + narrative | Submitted CBA; ICT Manager + Finance review |
| D1.3 | Prioritised action plan and cloud architecture recommendation | Written plan + architecture overview | Submitted plan; ICT Manager review |
...
| Ref | Deliverable | Format | Acceptance evidence |
|---|---|---|---|
| D2.1 | Cloud foundation environment built per the approved action plan and Solution Design | Running cloud infrastructure | Smoke-test evidence; ICT Manager review of deployed environment |
...
This Agreement commences on the Effective Date and continues until Acceptance of Deliverable D2.3 (Phase 2 operational handover) or earlier termination under this clause.
...
|---|---:|
| Phase 1 — Analysis and Business Case | $42,000 |
| Phase 2 — Cloud Foundation Build | $78,000 |
...
**Executed as an Agreement on 4 March 2026.**
